...
## ⚠ 設計原則は改訂審議中(2026-08-15・最優先で読む)

**現行の ARCH-001/002 は Codex 10ラウンドレビューで FATAL 指摘を受け、改訂案が
`criteria/_drafts/owner-ruling-arch-revision-{1,2}.json` に投入済み(オーナー承認待ち)**。
指摘の要点: ①「土台が下半分を代替する」という未確認の将来予測を原則に昇格させている
②信頼性の修復(成果欠落・重複実行・状態不整合・再開不能の防止)まで禁止する読み方を許す。

**正しい切り方は「配管かどうか」ではなく「実行メカニズム(委譲可)/制御意味論(orgh所有)」。**
方向性・機能の取捨・実行順の全体像は **`docs/strategy/direction-2026-08.md`** を読むこと
(同文書は Phase 0 完了まで実行開始 BLOCKED)。

Phase 0 の完了条件: ①旧ARCH-001/002行の失効 ②本セクションの更新 ③Reviewerへ旧規範が
注入されないことのテスト確認。

### (参考)現行の ARCH-001/002 = 上半分/下半分の境界(2026-08-14 オーナー裁定)

orghの機能は**上半分**(検収ゲート・criteria台帳と代謝・人間をノードに含むDAG・監査可能性)と
**下半分**(並列制御・プロセス隔離・キュー・スケジューリング=タスク実行の配管)に区別する。

- **下半分は土台ハーネス(Claude Code等)が将来ネイティブに備える領域**。新規実装は原則せず、
  adapter委譲を第一選択とする。既存実装も土台が同等機能を出した時点で削除・委譲を検討する
  (**作るより消すを正とする**)
- 機能追加の計画時は「土台の次版でこれが提供されたら不要になるか」を先に判定する(ARCH-002)
- 台帳: `criteria/arch.md` の **ARCH-001 / ARCH-002**(norm)
- 根拠と競争環境の分析: `docs/strategy/harness-landscape-2026-08.md`
- 注記: R-1(queue/executor)・R-2(slots)は下半分に該当する。当座の実害を解いた橋であり、
  いずれ捨てる前提の資産として扱う
