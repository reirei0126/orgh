...
async function mockStartOrApprove(existingId: string | null, intentForLog: string | null): Promise<string> {
  ...
  if (!existingId) {
    ...
    mockEmit<MissionLogEvent>("mission-log", { missionId: null, line: `intent: ${intentForLog}` });
    mockEmit<MissionLogEvent>("mission-log", { missionId: null, line: `mission ${missionId}: 3 tasks` });
